...
class PageResult:
    ...
    def to_json(self) -> str:
        # asdict() stays in sync automatically if fields are added later
...
class LinksOutput(BaseOutput):
    ...
    A link is "internal" if its netloc matches the base_domain OR is empty
    (relative URL). Subdomains of base_domain are treated as external unless
    they exactly match.
    ...
    base_domain      : Resolved automatically by OutputManager from start_url.
                       Can be overridden by passing it directly.
    """
    name = "links"
    ...
    def __init__(self, include_internal=True, include_external=True, base_domain=None):
        ...
        self.base_domain      = base_domain   # set by OutputManager if not provided
    ...
    def extract(self, content, raw_soup, response):
        ...
        for tag in content.find_all("a", href=True):
            ...
            # empty netloc = relative URL = always internal
            is_internal = netloc == "" or netloc == self.base_domain
...
class OutputManager:
    ...
    def __init__(self, outputs, start_url=None):
        """
        outputs   : list of str names or pre-configured BaseOutput instances.
        start_url : used to set base_domain on LinksOutput automatically.

        Examples
        --------
        OutputManager(["markdown", "metadata"], start_url="https://example.com")
        ...
        # inject base_domain into any LinksOutput that doesn't have one set
        for o in self.outputs:
            if isinstance(o, LinksOutput) and o.base_domain is None:
                o.base_domain = self.base_domain
